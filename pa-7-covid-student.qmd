...
Now use the `state_totals` and identify the top ten states/territories with the most cases between January 2020 and March 2023. Then make a graph ordered by greatest number of cases to least number of cases (Hint: use `forcats` functions to help you).
...
state_totals |> 
  ...
  slice(1:10) |>
  ggplot() + 
  geom_col(mapping = aes(x = total, y = fct_reorder(state, total))) + 
  labs(x = "U.S State",
       y = "Total Cases",
       title = "Top 10 States with Most COVID Cases (1/2020 - 3/2023)")
```
...
No, because higher population density & state size could skew the data
...
state_totals |> 
  ...
  mutate(rate = total/estimate) |>
  arrange(desc(rate)) |>
  slice(1:10) |>
  ggplot() +
  geom_col(mapping = aes(x = rate, y = fct_reorder(state, rate))) + 
  labs(x = "Relative Rate By Population",
       y = "US State",
       title = "Top 10 States with Most COVID Cases Relative to Population Size (1/2020 - 3/2023)")
...
> Case reporting may be inaccurate especially early on due to people not knowing what COVID was.
...
> Why did Alaska have so many COVID cases relative to the number of people there? Answering that would probably require a survey or a dataset exclusive to Alaska.   
